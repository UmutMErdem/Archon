...
def get_short_name(persona_file, mappings):
    ...
    return mappings.get(persona_file, persona_file.replace(".md", "").replace("_", " ").title())
...
def main():
    ...
    for pf in persona_files:
        ...
        source_domain = mappings.get(pf, pf.replace(".md", "").replace("_", " ").title())
        ...
        for line in lines:
            ...
            if m:
                ...
                target_presentation = get_short_name(target_file, mappings) if target_file else target_domain
                    
                interfaces.append((source_presentation, target_presentation, desc, pf, target_domain))
    ...
    if os.path.exists(MATRIX_PATH):
        ...
        diag_match = re.search(r"(## Relationship Diagram.*?)(?=\n---\n|\Z)", existing_content, re.DOTALL)
        if diag_match:
            relationship_diagram = diag_match.group(1).rstrip() + "\n"
    
    # 4. Write MATRIX
    new_matrix_text = "# Cross-Domain Interface Matrix\n\n"
    new_matrix_text += "> Single source of truth for all persona-to-persona interface contracts.\n"
    ...
    new_matrix_text += "Run `python scripts/validate_manifesto.py` to verify bidirectional symmetry.\n"
