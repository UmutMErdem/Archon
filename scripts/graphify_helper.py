...
def install_graphify():
    ...
    if has_uv:
        print("Detected 'uv'. Installing 'graphifyy' tool via uv with matplotlib support...")
        try:
            subprocess.run(["uv", "tool", "install", "graphifyy", "--with", "matplotlib"], check=True)
    ...
    try:
        subprocess.run([sys.executable, "-m", "pip", "install", "graphifyy", "matplotlib"], check=True)
...
def main():
    parser = argparse.ArgumentParser(description="Graphify Automation and Helper Script")
    parser.add_argument("--path", default=".", help="Target path to analyze (default: current directory)")
    parser.add_argument("--build", action="store_true", help="Build/rebuild the knowledge graph")
    parser.add_argument("--update", action="store_true", help="Perform incremental update of the graph")
    parser.add_argument("--query", type=str, help="Query/Ask a question to the knowledge graph")
    parser.add_argument("--hooks", action="store_true", help="Install Git hooks for automatic updates")
    parser.add_argument("--install-only", action="store_true", help="Only install graphifyy and exit")
    ...
    if args.build or args.update or len(sys.argv) == 1:
        ...
        obsidian_cmd = ["graphify", "export", "obsidian", "--graph", graph_json_path, "--dir", obsidian_dir]
        print(f"Running Obsidian export command: {' '.join(obsidian_cmd)}")
        subprocess.run(obsidian_cmd, capture_output=True)  # Run silently to avoid cluttering logs on expected minor errors
